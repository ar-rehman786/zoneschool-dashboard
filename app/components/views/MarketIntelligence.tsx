...
function DonutChart({ title, data }: { title: string; data: DonutDatum[] }) {
  ...
  if (nonZero.length <= 1) {
    return (
      <div>
        <p className="uppercase tracking-wide mb-3 text-center" style={{ fontSize: '12px', color: t.textMuted, fontFamily: C.fontHeading, fontWeight: 600 }}>{title}</p>
        <div className="flex flex-col items-center justify-center py-6">
          ...
          <span style={{ fontFamily: C.fontBody, fontSize: '13px', color: t.textMuted, marginTop: '4px' }}>100% of respondents</span>
        </div>
        <div className="flex flex-wrap justify-center gap-3 mt-1">
          {data.map(d => (
            <div key={d.name} className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full" style={{ background: d.color, opacity: d.value > 0 ? 1 : 0.3 }} />
              <span style={{ fontSize: '12px', color: d.value > 0 ? t.body : t.textMuted, fontFamily: C.fontBody }}>{d.name} ({d.value})</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="relative" style={{ width: 160, height: 160, margin: '0 auto' }}>
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Tooltip
              contentStyle={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: '10px', fontFamily: 'Questrial', fontSize: '13px', boxShadow: t.cardShadow }}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          <span style={{ fontFamily: C.fontBody, fontSize: '11px', color: t.textMuted }}>{dominant.name}</span>
        </div>
      </div>
...
export function MarketIntelligence({ leads }: { leads: Lead[] }) {
  ...
  leads.forEach(l => l.identity_tags.forEach(tag => {
    if (isValidIdentityTag(tag)) {
    ...
    <div className="animate-fade-in">
      ...
      <Card className="mb-6">
        ...
        <div className="h-[200px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={urgencyData} layout="vertical" barSize={28}>
              <CartesianGrid strokeDasharray="3 3" stroke={t.isDark ? '#2A3142' : '#F1F5F9'} horizontal={false} />
              <XAxis type="number" tick={{ fontSize: 11, fill: t.textMuted, fontFamily: 'Questrial' }} axisLine={false} tickLine={false} allowDecimals={false} />
              <YAxis type="category" dataKey="name" tick={{ fontSize: 13, fill: t.text, fontWeight: 600, fontFamily: 'Outfit' }} axisLine={false} tickLine={false} width={55} />
              <Tooltip
                contentStyle={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: '12px', fontFamily: 'Questrial', fontSize: '13px', boxShadow: t.cardShadow }}
                cursor={{ fill: t.isDark ? 'rgba(255,255,255,0.03)' : 'rgba(30,36,48,0.03)' }}
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {urgencyData.map(d => (
            <span key={d.name} className="px-3 py-1.5 rounded-lg text-xs" style={{ background: `${d.fill}${t.isDark ? '20' : '12'}`, color: d.fill, fontFamily: C.fontHeading, fontWeight: 600 }}>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-1">
          ...
          <span style={{ fontSize: '12px', color: t.textMuted, fontFamily: C.fontBody }}>From {hotLeads.length} hot lead{hotLeads.length !== 1 ? 's' : ''}</span>
        </div>
        <p className="mb-4" style={{ fontSize: '13px', color: t.textMuted, fontFamily: C.fontBody, marginTop: '-12px' }}>Language from your hottest prospects</p>
          ...
          {boxes.map(b => (
            <Card key={b.title} className="overflow-hidden p-0">
              <div style={{ borderTop: `3px solid ${b.color}` }} className="p-5">
                ...
                {b.items.length === 0 ? (
                  <p className="italic" style={{ fontSize: '13px', color: t.textMuted, fontFamily: C.fontBody }}>No data yet</p>
                ) : (
                  <ul className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                    {b.items.slice(0, 12).map((item, i) => (
                      <li key={i} className="flex items-start gap-2 leading-relaxed" style={{ color: t.body, fontFamily: C.fontBody, fontSize: '13px' }}>
      ...
      <div className="mb-6">
        ...
        <p className="mb-4" style={{ fontSize: '13px', color: t.textMuted, fontFamily: C.fontBody, marginTop: '-12px' }}>Real objections and blockers from your respondents</p>
        {objectionItems.length === 0 ? (
          <Card>
            <p className="text-center py-6" style={{ fontSize: '14px', color: t.textMuted, fontFamily: C.fontBody }}>No objections recorded yet</p>
          </Card>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            {objectionItems.slice(0, 12).map((ob, i) => (
              <div key={i} className="rounded-2xl p-4" style={{ background: t.card, border: `1px solid ${t.cardBorder}`, boxShadow: t.cardShadow }}>
                <p className="line-clamp-3 mb-2" style={{ fontFamily: C.fontBody, fontSize: '13px', color: t.body, lineHeight: '1.6' }}>&ldquo;{ob.text}&rdquo;</p>
                <div className="flex items-center justify-between">
                  <span style={{ fontFamily: C.fontBody, fontSize: '12px', color: t.textMuted }}>{ob.leadName}</span>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
          <Card>
            <p className="uppercase tracking-wide mb-3" style={{ fontSize: '12px', color: t.textMuted, fontFamily: C.fontHeading, fontWeight: 600 }}>Identity Tags</p>
            {sortedTags.length === 0 ? (
              <p className="italic" style={{ fontSize: '13px', color: t.textMuted, fontFamily: C.fontBody }}>No tags yet</p>
            ) : (
              <div className="flex flex-wrap gap-2">
                {sortedTags.map(([tag, count]) => (
                  <span key={tag} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full"
                    style={{ background: t.card, border: `1px solid ${t.cardBorder}`, fontFamily: C.fontHeading, fontSize: '13px', fontWeight: 500, color: t.text }}>
                    {tag}
      ...
      <div className="mb-6">
        ...
        {phrases.length === 0 ? (
          <Card><p className="text-center py-6" style={{ fontSize: '14px', color: t.textMuted, fontFamily: C.fontBody }}>No key phrases extracted yet.</p></Card>
        ) : (
          <div className="flex flex-wrap gap-3">
            {phrases.map((p, i) => (
              <div key={i} className="relative rounded-2xl p-5 overflow-hidden w-full lg:w-auto"
                ...
                <p className="relative italic" style={{ fontFamily: C.fontBody, fontSize: '14px', color: t.body, paddingLeft: '4px', lineHeight: '1.6' }}>{p.phrase}</p>
                <p className="mt-2" style={{ fontSize: '12px', color: t.textMuted, fontFamily: C.fontHeading, fontWeight: 500 }}>&mdash; {p.name}</p>
              </div>
            ))}
          </div>
        )}
